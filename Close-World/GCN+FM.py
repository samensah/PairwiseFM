...
class ConMask:
    ...
    def testing_data_open_tail_test_head(self, batch_size=100):
        ...
        while start < n_triple:
            ...
            start = end
    def testing_data_close_test(self, batch_size=100):
        n_triple = len(self.__close_test)
        start = 0
        while start < n_triple:
            end = min(start + batch_size, n_triple)
            yield self.__close_test[start:end, :]
            start = end
...
# ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        self.__close_test = []
        for i in range(self.__test_triple.shape[0]):
            if self.__test_triple[i][0] in self.__close_id_entity_map and self.__test_triple[i][1] in self.__close_id_entity_map:
                self.__close_test.append(self.__test_triple[i])

        self.__close_test = np.asarray(self.__close_test)
        print ('close_test: ', len(self.__close_test))
...
def main(_):
    ...
    with tf.Session(config=config) as session:
        ...
        for i in range(args.n_worker):
            worker = Process(target=worker_func, args=(evaluation_queue, result_queue, model.r_t, model.r_h, model.hr_t, model.tr_h))
            worker.start()
        ######################################################################################
        ...
        for n_iter in range(iter_offset, args.max_iter):
            ...
            if n_iter % args.eval_per == 0 or n_iter == args.max_iter - 1:
                ...
                for data_func, test_type in zip([model.testing_data_close_test, model.testing_data_close_test], ['tail', 'head']):
                    ...
                    for testing_data in data_func(batch_size=args.eval_batch):
                        ...
                        if test_type == 'tail':
                            ...
                            for index in range(len(r)):
                                ...
                                # temp_candidate = list(set(range(model.n_close_ent)) - model.hr_t[h[index][0]][r[index][0]])
                        ...
                        if test_type == 'head':
                            ...
                            for index in range(len(r)):
                                ...
                                # temp_candidate = list(set(range(model.n_close_ent)) - model.tr_h[t[index][0]][r[index][0]])
